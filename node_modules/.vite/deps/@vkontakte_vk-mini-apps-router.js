...
import src_bridge from "./@vkontakte_vk-bridge.js";
import { gt as Link } from "./dist-DS5LPutx.js";
...
function createBrowserHistory(options) {
	...
	return getUrlBasedHistory$1(createBrowserLocation, createBrowserHref, null, options);
}
/**
...
function createHashHistory(options) {
	...
	return getUrlBasedHistory$1(createHashLocation, createHashHref, validateHashLocation, options);
}
...
*/
function getHistoryState$1(location, index) {
	return {
		usr: location.state,
		key: location.key,
		idx: index
...
function getUrlBasedHistory$1(getLocation, createHref, validateLocation, options) {
	if (options === void 0) options = {};
	...
	function push(to, state) {
		action = Action.Push;
		let location = createLocation$1(history.location, to, state);
		if (validateLocation) validateLocation(location, to);
		index = getIndex() + 1;
		let historyState = getHistoryState$1(location, index);
...
function convertRoutesToDataRoutes(routes, mapRouteProperties, parentPath, manifest) {
	...
	return routes.map((route, index) => {
		...
		if (isIndexRoute(route)) {
			let indexRoute = _extends({}, route, mapRouteProperties(route), { id });
...
function createRouter(init) {
	...
	async function startNavigation(historyAction, location, opts) {
		...
		pendingNavigationController = null;
		completeNavigation(location, _extends({ matches: updatedMatches || matches }, getActionDataForCommit(pendingActionResult), {
			loaderData,
			errors
		}));
	}
	...
	async function handleLoaders(request, location, matches, isFogOfWar, overrideNavigation, submission, fetcherSubmission, replace, initialHydration, flushSync, pendingActionResult) {
		...
		if (isFogOfWar) {
			if (shouldUpdateNavigationState) {
				...
				updateState(_extends({ navigation: loadingNavigation }, actionData !== void 0 ? { actionData } : {}), { flushSync });
			}
			let discoverResult = await discoverRoutes(matches, location.pathname, request.signal);
			if (discoverResult.type === "aborted") return { shortCircuited: true };
			else if (discoverResult.type === "error") {
	...
	*/
	async function startRedirectNavigation(request, redirect, isNavigation, _temp2) {
		...
		if (redirectPreserveMethodStatusCodes.has(redirect.response.status) && activeSubmission && isMutationMethod(activeSubmission.formMethod)) await startNavigation(redirectHistoryAction, redirectLocation, {
			submission: _extends({}, activeSubmission, { formAction: location }),
...
function getMatchesToLoad(history, state, matches, submission, location, initialHydration, skipActionErrorRevalidation, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, deletedFetchers, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, pendingActionResult) {
	...
	fetchLoadMatches.forEach((f, key) => {
		...
		else shouldRevalidate = shouldRevalidateLoader(fetcherMatch, _extends({
			currentUrl,
...
async function loadLazyRouteModule(route, mapRouteProperties, manifest) {
	...
	Object.assign(routeToUpdate, _extends({}, mapRouteProperties(routeToUpdate), { lazy: void 0 }));
}
...
function processLoaderData(state, matches, results, pendingActionResult, revalidatingFetchers, fetcherResults, activeDeferreds) {
	...
	revalidatingFetchers.forEach((rf) => {
		...
		if (controller && controller.signal.aborted) return;
			...
			if (!(errors && errors[boundaryMatch.route.id])) errors = _extends({}, errors, { [boundaryMatch.route.id]: result.error });
			state.fetchers.delete(key);
		} else if (isRedirectResult(result)) invariant$1(false, "Unhandled fetcher revalidation redirect");
		else if (isDeferredResult(result)) invariant$1(false, "Unhandled fetcher deferred data");
		else {
...
function stripHashFromPath(path) {
	...
	return createPath(_extends({}, parsedPath, { hash: "" }));
}
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/const.js
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/contexts.js
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/utils/utils.js
...
function isModalShown(location) {
	return location.state && "showModal" in location.state;
}
function isPopoutShown(location) {
	return location.state && "showPopout" in location.state;
}
function createKey() {
...
function transformSearchParams(searchParams = "") {
	if (!isString(searchParams) && !(searchParams instanceof URLSearchParams)) return `${new URLSearchParams(searchParams)}`;
...
function getPathFromTo({ to, params, defaultPathname = "" }) {
	...
	if (getParamKeys(path).length > 0) {
		const filledPath = fillParamsIntoPath(path, params);
		return isPageWithOptionsPath(to) ? createPath({
			...to,
			pathname: filledPath,
			search
		}) : filledPath;
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/utils/buildPanelPathFromModalMatch.js
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/services/InitialLocation.js
var InitialLocation = class InitialLocation {
	constructor(value) {
		this._value = value;
	}
	static init(value) {
		if (!InitialLocation._instance) InitialLocation._instance = new InitialLocation(value);
	}
	static get value() {
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/utils/createBrowserRouter.js
function createBrowserRouter(routes) {
	window.addEventListener("popstate", (event) => {
		if (!event.state) window.history.replaceState({ key: createKey() }, "");
	});
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/utils/react-router-override/createLocation.js
/**
* Creates a Location object with a unique key from the given Path
*/
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/utils/react-router-override/getHistoryState.js
/**
* For browser-based histories, we combine the state and key into an object
*/
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/utils/react-router-override/getUrlBasedHistory.js
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/utils/react-router-override/HashParamHistory.js
/**
...
function createHashParamHistory(options = {}) {
	...
	function createHashParamLocation(window, globalHistory) {
		const { pathname = "/", search = "", hash = "" } = parsePath(new URLSearchParams(window.location.hash.substring(1)).get(paramName) || "");
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/utils/createHashParamRouter.js
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/utils/createHashRouter.js
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/utils/createSearchParams.js
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/services/TransactionExecutor.js
var TransactionExecutor = class TransactionExecutor {
	constructor() {
		this.transactions = [];
	}
	static getInstance() {
		if (!TransactionExecutor.instance) TransactionExecutor.instance = new TransactionExecutor();
	...
	static add(transaction) {
		TransactionExecutor.getInstance().transactions.push(transaction);
	}
	...
	static async doNext() {
		const transactions = TransactionExecutor.getInstance().transactions;
		if (transactions.length) {
			transactions[0].doNext();
			if (transactions[0]?.finished) transactions.shift();
		}
	}
};
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/entities/NavigationTransaction.js
var NavigationTransaction = class {
	...
	doNext() {
		...
		if (this.finished) this.resolve();
	}
};
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/utils/fillHistory.js
function flattenBranch(leafs, parents) {
	return leafs.map((leaf) => {
		const leafWithParents = {
			...leaf,
			parents
		};
...
function fillHistory(config, routeNavigator, context) {
	...
	if (!potentialLeafs.length) return;
	const records = (potentialLeafs.find((leaf) => !leaf.children?.length) ?? potentialLeafs[0]).parents.map(({ path }) => fillParamsIntoPath(path, params));
	setTimeout(() => {
		if (records.length) {
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/utils/getHrefWithoutHash.js
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/services/BridgeService.js
var BridgeService = class BridgeService {
	static {
		this.UNSUPPORTED_PLATFORM_ERROR_CODE = 6;
	}
	constructor() {
	...
	static {
		this.handlePlatformError = (error) => {
			if (error.error_data.error_code !== BridgeService.UNSUPPORTED_PLATFORM_ERROR_CODE) console.log(error);
		};
	}
	static enableNativeSwipeBack() {
		Promise.all([src_bridge.send("VKWebAppSetSwipeSettings", { history: true }), src_bridge.send("VKWebAppEnableSwipeBack")]).catch(BridgeService.handlePlatformError);
	...
	static disableNativeSwipeBack(consumerId) {
		...
		*/
		if (instance.swipeBackConsumers.length === 0) Promise.all([src_bridge.send("VKWebAppSetSwipeSettings", { history: false }), src_bridge.send("VKWebAppDisableSwipeBack")]).catch(BridgeService.handlePlatformError);
	}
	static get instance() {
		if (!BridgeService._instance) BridgeService._instance = new BridgeService();
		return BridgeService._instance;
	}
};
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/services/EventBus.js
var EventBus = class EventBus {
	...
	static broadcast(contextName, args) {
		...
		if (!eventBus.isEventExist(contextName)) return;
		eventBus.subscriptions[contextName].forEach((callback) => callback(...args));
	}
	static subscribe(contextName, callback) {
		const eventBus = EventBus.getInstance();
		if (!eventBus.isEventExist(contextName)) eventBus.subscriptions[contextName] = /* @__PURE__ */ new Set();
		const callbacks = eventBus.subscriptions[contextName];
		callbacks.add(callback);
		return () => {
			callbacks.delete(callback);
			if (callbacks.size === 0) delete eventBus.subscriptions[contextName];
		};
	}
};
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/services/ContextThrottleService.js
var ContextThrottleService = class ContextThrottleService {
	constructor() {
		this.interval = 0;
		this.throttled = true;
	...
	static getInstance() {
		if (!ContextThrottleService.instance) ContextThrottleService.instance = new ContextThrottleService();
	...
	throttleUpdateContextValue(contextName, newValue) {
		...
		if (TransactionExecutor.isRunSyncActive) return;
		...
		if (timeUntilNextUpdate <= 0) this.updateContextValue(contextName, newValue);
		else contextData.updateTimerId = setTimeout(() => {
	...
	static triggerContextUpdate(contextName, newValue) {
		...
		if (newValue === throttledService.getWithInitThrottleInfoByName(contextName, newValue).throttledValue) return;
		if (!throttledService.throttled && !TransactionExecutor.isRunSyncActive) throttledService.updateContextValue(contextName, newValue);
		else throttledService.throttleUpdateContextValue(contextName, newValue);
	}
	static retrieveContextInfo(contextName, contextValue) {
		const { prevValue, throttledValue } = ContextThrottleService.getInstance().getWithInitThrottleInfoByName(contextName, contextValue);
		return {
			prevValue,
			throttledValue
		};
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/services/DefaultRouteNavigator.js
var DefaultRouteNavigator = class {
	...
	async backToFirst() {
		if (this.viewHistory.position > 0) await this.go(-this.viewHistory.position);
		else await TransactionExecutor.doNext();
	}
	async go(to) {
		if (to === 0) await TransactionExecutor.doNext();
		else await this.router.navigate(to);
	}
	runSync(actions) {
	...
	async hideModal(pushPanel = false, options) {
		if (!pushPanel && !this.viewHistory.isFirstPage || isModalShown(this.router.state.location)) return await this.router.navigate(-1);
	...
	async showPopout(popout) {
		...
		if (isModalShown(this.router.state.location)) state[STATE_KEY_SHOW_MODAL] = this.router.state.location.state[STATE_KEY_SHOW_MODAL];
	...
	async hidePopout() {
		if (isPopoutShown(this.router.state.location)) {
			this.setPopout(null);
			if (isModalShown(this.router.state.location)) await this.router.navigate(this.router.state.location, {
				state: {
					[STATE_KEY_BLOCK_FORWARD_NAVIGATION]: true,
			...
			else await this.router.navigate(-1);
		} else await TransactionExecutor.doNext();
	}
	block(blocker) {
	...
	async navigate(to, opts, params = {}) {
		let path = getPathFromTo({
			to,
			params,
			defaultPathname: this.router.state.location.pathname
		});
	...
	validateOptions({ state, keepSearchParams } = {}) {
		if (state && typeof state !== "object" || keepSearchParams && typeof keepSearchParams !== "boolean") {
	...
	parseParams(to, paramsOrOptions = {}, options = {}) {
		if (getParamKeys(extractPathFromNavigationTarget(to)).length) return {
			preparedParams: paramsOrOptions,
			preparedOptions: this.validateOptions(options)
		};
		return {
			preparedParams: {},
			preparedOptions: this.validateOptions(paramsOrOptions)
		};
	}
};
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/services/ViewHistory.js
var ViewHistory = class {
	...
	updateNavigation(state) {
		...
		switch (state.historyAction) {
			case Action.Push:
				...
				break;
			case Action.Pop:
				if (this.hasKey(state.location.key)) this.pop(state);
				else this.push(record);
				break;
			case Action.Replace: this.replace(record);
		}
	}
	get isFirstPage() {
	...
	get panelsHistory() {
		...
		return reversedClone.slice(0, rightLimit > -1 ? rightLimit : reversedClone.length).filter((item) => !item.modal && !item.popout).reverse().map(({ panel }) => panel);
	}
	get position() {
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/services/HistoryManager.js
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/hooks/useBlockForwardToModals.js
var processedKeys = [];
function useBlockForwardToModals(router, viewHistory, routeNavigator) {
	(0, import_react.useEffect)(() => {
		const blockerFunction = ({ historyAction, nextLocation }) => {
			const blockEnabled = viewHistory.isPopForward(historyAction, nextLocation.key) && nextLocation.key !== "default";
			return Boolean(blockEnabled && nextLocation.state?.["blockForward"]);
		};
	...
	(0, import_react.useEffect)(() => {
		router.subscribe((state) => {
			...
			if (viewHistory.isPopBackward(state.historyAction, key) && state.location.state?.["blockForward"] && !processedKeys.includes(key)) {
				processedKeys.push(key);
				const replaceState = { ...window.history.state };
				if (replaceState.usr?.["showModal"]) {
				...
				if (replaceState.usr?.["showPopout"]) {
					replaceState.usr = { ...replaceState.usr };
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/components/DefaultNotFound.js
var DivStyles = {
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/components/RouterProvider.js
function RouterProvider({ router, children, notFound, hierarchy, notFoundRedirectPath, interval = 400, useBridge = true, throttled = true }) {
	const [popout, setPopout] = (0, import_react.useState)(null);
	const [viewHistory] = (0, import_react.useState)(new ViewHistory());
	const [panelsHistory, setPanelsHistory] = (0, import_react.useState)([]);
	const isPopoutShown = router.state.location.state?.[STATE_KEY_SHOW_POPOUT];
	const dataRouterContext = (0, import_react.useMemo)(() => {
		return {
			router,
			routeNavigator: new DefaultRouteNavigator(router, viewHistory, setPopout),
			viewHistory
		};
	}, [
		router,
	...
	const routeContext = (0, import_react.useMemo)(() => getRouteContext(router.state, panelsHistory), [router.state, panelsHistory]);
	const dataPopoutContext = (0, import_react.useMemo)(() => {
	...
	(0, import_react.useEffect)(() => {
		...
		if (useBridge) {
			src_bridge.subscribe(({ detail }) => {
				if (detail.type !== "VKWebAppChangeFragment") return;
				const location = detail.data.location;
				const to = new URLSearchParams(location).get("path") || location || "/";
			...
			router.subscribe((state) => {
				...
				src_bridge.send("VKWebAppSetLocation", {
					location,
					replace_state: true
				});
			});
		...
		if (hierarchy && enableFilling) fillHistory(hierarchy, dataRouterContext.routeNavigator, routeContext);
	}, [router]);
	...
	if (notFoundRedirectPath && (routeNotFound || routeContext.match?.route.path === "*")) {
		if (router.state.location.pathname === notFoundRedirectPath) console.warn("Incorrect notFoundRedirectPath");
	...
	return (0, import_jsx_runtime.jsx)(RouterContext.Provider, {
		...
		children: (0, import_jsx_runtime.jsxs)(PopoutContext.Provider, {
			value: dataPopoutContext,
			children: [routeNotFound && (notFound || (0, import_jsx_runtime.jsx)(DefaultNotFound, { routeNavigator: dataRouterContext.routeNavigator })), !routeNotFound && (0, import_jsx_runtime.jsx)(RouteContext.Provider, {
				value: routeContext,
				children
			})]
		})
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/hooks/useThrottledContext.js
function useThrottledContext(context) {
	...
	const [prevValue, setPrevValue] = (0, import_react.useState)(initialContextData.prevValue);
	const [throttledValue, setThrottledValue] = (0, import_react.useState)(initialContextData.throttledValue);
	(0, import_react.useEffect)(() => {
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/hooks/hooks.js
function useRouteNavigator() {
	const routerContext = (0, import_react.useContext)(RouterContext);
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/hooks/useResolvedPath.js
/**
* @private
*
* When processing relative navigation we want to ignore ancestor routes that
...
function useResolvedPath(to, { relative } = {}) {
	const { router: { state: { matches } } } = (0, import_react.useContext)(RouterContext);
	const { pathname: locationPathname } = useLocation();
	const routePathnamesJson = JSON.stringify(getPathContributingMatches(matches).map((match) => match.pathnameBase));
	return (0, import_react.useMemo)(() => resolveTo(to, JSON.parse(routePathnamesJson), locationPathname, relative === "path"), [
		to,
		routePathnamesJson,
		locationPathname,
		relative
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/hooks/useHref.js
function useHref(to, { relative, params }) {
	const routeContext = (0, import_react.useContext)(RouterContext);
	invariant(routeContext, "You can not use useHref hook outside of RouteContext. Make sure calling it inside RouterProvider.");
	const { hash, pathname, search } = useResolvedPath(getPathFromTo({
		to,
		params,
		defaultPathname: routeContext.router.state.location.pathname
	}), { relative });
	const hrefWithoutHash = getHrefWithoutHash();
	const location = routeContext.router.createHref({
		pathname,
		search,
		hash
	}).replace(hrefWithoutHash, "");
	return location.startsWith("/") ? location : "/" + location;
}
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/hooks/useLinkClickHandler.js
...
function useLinkClickHandler(to, { target, replace: replaceProp, preventScrollReset, relative, params }) {
	...
	const path = useResolvedPath(getPathFromTo({
		to,
		params,
		defaultPathname: location.pathname
	}), { relative });
	return (0, import_react.useCallback)((event) => {
		if (shouldProcessLinkClick(event, target)) {
			event.preventDefault();
			const toPath = createPath(path);
			if (replaceProp !== void 0 ? replaceProp : createPath(location) === toPath) navigator.replace(toPath);
			else navigator.push(toPath);
		}
	}, [
		location,
		navigator,
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/components/RouterLink.js
...
var RouterLinkInner = ({ to, relative, replace, target, reloadDocument, params, onClick, ...rest }, ref) => {
	...
	if (isString(to) && ABSOLUTE_URL_REGEX.test(to)) {
		...
		if (isBrowser) {
			...
			if (targetUrl.origin === currentUrl.origin) navigationTarget = path + targetUrl.search + targetUrl.hash;
			else isExternal = true;
		}
	}
	const href = useHref(navigationTarget, {
		relative,
		params
	});
...
var RouterLink = (0, import_react.forwardRef)(RouterLinkInner);
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/hooks/useHistoryManager.js
function useHistoryManager() {
	const { viewHistory } = (0, import_react.useContext)(RouterContext);
	const historyManager = (0, import_react.useMemo)(() => new HistoryManager(viewHistory), [viewHistory]);
	invariant(viewHistory, "You can not use useHistoryManager hook outside of RouteContext. Make sure calling it inside RouterProvider.");
	return historyManager;
}
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/hooks/useParams.js
/**
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/hooks/useGetPanelForView.js
function useGetPanelForView(view) {
	...
	const route = match?.route;
	return (view && prevRouteContext?.match?.route.view === view && route?.view !== view ? prevRouteContext.match.route : route)?.panel;
}
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/hooks/useSearchParams.js
/**
...
function useSearchParams(defaultInit) {
	...
	let hasSetSearchParamsRef = (0, import_react.useRef)(false);
	let location = useLocation();
	let searchParams = (0, import_react.useMemo)(() => getSearchParamsForLocation(location.search, hasSetSearchParamsRef.current ? null : defaultSearchParamsRef.current), [location.search]);
	let router = (0, import_react.useContext)(RouterContext).router;
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/hooks/useMetaParams.js
/**
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/hooks/useFirstPageCheck.js
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/hooks/useActiveVkuiLocation.js
function useActiveVkuiLocation() {
	const [routeContext] = useThrottledContext(RouteContext);
	const popout = usePopout();
	const { match, state, panelsHistory } = routeContext;
	const route = match?.route;
	const modal = state.location.state?.["showModal"] ?? (route && "modal" in route ? route.modal : void 0);
	return {
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/hooks/useEnableSwipeBack.js
function useEnableSwipeBack() {
	const consumerId = (0, import_react.useRef)(null);
	(0, import_react.useEffect)(() => {
		consumerId.current = BridgeService.enableNativeSwipeBack();
		return () => {
			if (consumerId.current !== null) BridgeService.disableNativeSwipeBack(consumerId.current);
		};
	}, []);
}
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/hooks/useBlocker.js
function useBlocker(blocker) {
	const routeNavigator = useRouteNavigator();
	const [routeContext] = useThrottledContext(RouteContext);
	const unblocker = (0, import_react.useRef)();
	(0, import_react.useEffect)(() => {
	...
	return routeContext.state.blockers.get("vk-mini-app-navigation-block") ?? IDLE_BLOCKER;
}
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/hoc/withRouter.js
/**
...
function withRouter(Component) {
	function WithRouter(props) {
		...
		return (0, import_jsx_runtime.jsx)(Component, { ...propsWithRouter });
	}
	WithRouter.displayName = `WithRouter(${getDisplayName(Component)})`;
	return WithRouter;
}
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/page-types/common.js
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/page-types/ViewConfig.js
var ViewConfig = class {
	...
	getRoutes() {
		return this.panels.map((panel) => panel.getRoutes()).flat().map((panelRoute) => ({
			...panelRoute,
			view: this.id
		}));
	}
};
function createView$1(id, panels) {
	return new ViewConfig(id, panels);
}
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/page-types/RootConfig.js
var RootConfig = class {
	constructor(id, views) {
		this.id = id;
		this.views = views;
		if (!views.length) throw new Error(`Trying to create root ${id} without views. Root must have at least one view.`);
		views.forEach((views) => {
	...
	getRoutes() {
		return this.views.map((view) => view.getRoutes()).flat().map((viewRoute) => ({
			...viewRoute,
			root: this.id
		}));
	}
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/page-types/PanelPage.js
var BasePanelPage = class {
	...
	getRoutes() {
		return this.modals.map((modalOrTab) => modalOrTab.getRoutes()).flat().map((route) => ({
			...route,
			panel: this.id
		})).concat({
			path: this.path,
...
function createPanel(id, path, modals, paramKeys) {
	if (paramKeys) return new PanelPageWithParams(id, path, paramKeys, modals ?? []);
	return new PanelPage(id, path, modals ?? []);
}
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/page-types/ModalPage.js
var ModalPage = class {
	constructor(id, path) {
		this.id = id;
		this.path = path;
...
function createModal(id, path, paramKeys) {
	if (paramKeys) return new ModalPageWithParams(id, path, paramKeys);
	return new ModalPage(id, path);
}
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/page-types/TabPage.js
var BaseTabPage = class {
	...
	getRoutes() {
		return this.modals.map((modal) => modal.getRoutes()).flat().map((route) => ({
			...route,
			tab: this.id
		})).concat({
			path: this.path,
...
function createTab(id, path, modals, paramKeys) {
	if (paramKeys) return new TabPageWithParams(id, path, paramKeys, modals ?? []);
	return new TabPage(id, path, modals ?? []);
}
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/page-types/RoutesConfig.js
var RoutesConfig = class RoutesConfig {
	constructor() {
		this.items = [];
	}
	getRoutes() {
...
//#endregion
//#region node_modules/@vkontakte/vk-mini-apps-router/dist/page-types/index.js
var createView = createView$1;
//#endregion
export { RouterLink, RouterProvider, RoutesConfig, createBrowserRouter, createHashParamRouter, createHashRouter, createModal, createPanel, createRoot, createTab, createView, getInitialLocation, useActiveVkuiLocation, useBlocker, useEnableSwipeBack, useFirstPageCheck, useGetPanelForView, useHistoryManager, useHref, useLinkClickHandler, useMetaParams, useParams, usePopout, useRouteNavigator, useSearchParams, withRouter };
